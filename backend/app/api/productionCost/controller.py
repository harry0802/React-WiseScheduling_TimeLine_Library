...
class ProductionCostController(Resource):
    ...
    @api.param("productionScheduleId", "製令單生產排程的ID，藉由這個ID，可以找到該製令單的母批和子批", required=False, type=int)
    ...
    def get(self):
        ...
        productionScheduleId = request.args.get('productionScheduleId', default=None, type=int)
        return ProductionCostService.get_productCosts(page=page, size=size, sort=sort, dataType=dataType, productionScheduleId=productionScheduleId)
