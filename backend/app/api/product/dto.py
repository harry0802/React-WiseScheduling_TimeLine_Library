...
class productDto:
    ...
    base_obj = {
        ...
        "oldProductSN": fields.String(required=False, description="Old ProductSN", example="Old PROD-00001-01"),
    }
    ...
    product_object = api.model(
        "product object", base_obj
    )

    #POST model
    post_obj = copy.deepcopy(base_obj)
    pop_list = ["id"]
    required_list = ["productSN", "productName"]
    not_required_list = []
    ...
    product_post = api.model("product post", post_obj)
    ...
    product_put = api.model("product put", put_obj)
    
    #response model
    product_resp = api.model(
        "product response",
        {
            "status": fields.Boolean,
            "message": fields.String,
            "data": fields.Nested(product_object),
        }
    )
