...
"""生產行程(ProductionSchedule)
    1. V生產區域 : productionArea (string)
    2. V機台編號 : machineSN (string)
    3. V序號 : serialNumber (string)
    4. V單據編號 : workOrderSN (string)
    5. V產品編號 : productSN (string)
    6. V產品名稱 : productName (string)
    7. V製令數量 : workOrderQuantity (int)
    8. V製令開立日期 : workOrderDate (date)
    9. V成型秒數 : moldingSecond (int)
    10. V每小時產能 : hourlyCapacity (int)(round)
    11. 日產能 : dailyCapacity (int)(round)
    12. V預計上機日 : planOnMachineDate (date)
    13. 實際上機日 : actualOnMachineDate (date)(nullable)
    13. V上下模工作日 : moldWorkDays (int)
    14. V工作天數 : workDays (int) (ceil)
    15. V預計完成日 : planFinishDate (date) (filled or calculated by calendarWithoutHoliday)
    16. V實際完成日 : actualFinishDate (nullable)
    17. V備註: comment (nullable)
    18. V日工時 : dailyWorkingHours (int)
    19. V穴數 : moldCavity (int)
    20. V週別 : week (int)
    21. V單雙色射出 : singleOrDoubleColor (string)
    22. V製程轉換率 : conversionRate (decimal)
    23. VStatus : status (string) 
