...
const coreUtils = {
  ...
  formatISOWithTZ: (date = new Date()) =>
    dayjs(date).format("YYYY-MM-DDHH:mm:ss.SSSZ"),
};
...
const workdayUtils = {
  ...
  /**
   * 計算給定日期的 ISO 週數
   * @param {Date|string|number} date - 要計算週數的日期
   * @returns {number} ISO 週數 (1-53)
   */
  getISOWeek: (date) => {
    const firstDayOfYear = dayjs(date).startOf("year");
    const firstThursday = firstDayOfYear.add(
      (4 - firstDayOfYear.day() + 7) % 7,
      "day"
    );
    const currentThursday = dayjs(date).startOf("week").add(3, "day");

    return Math.ceil(currentThursday.diff(firstThursday, "day") / 7) + 1;
  },
