...
const CostResultHandler = {
  ...
  getItemAmount(costSubtotalResult, dataKey, index) {
    ...
    */
    ...
    const amountMap = {
      // 運費
      FQMaterialCosts: costSubtotalResult?.materialCostResult?.amounts?.[index],
      // 包材費用
      FQPackagingCosts:
        costSubtotalResult?.packagingCostResult?.amounts?.[index],
      // 成型費用
      FQInjectionMoldingCosts: costSubtotalResult?.moldingCostResult,
      // 後製程費用
      FQInPostProcessingCosts:
      ...
      FQOutPostProcessingCosts:
        costSubtotalResult?.postProcessingCostResult?.amounts?.[index],
      // 運費
      FQFreights: costSubtotalResult?.transportAmounts?.[index],
      // 關稅
      FQCustomsDuties: costSubtotalResult?.freightAmounts?.[index],
    };
