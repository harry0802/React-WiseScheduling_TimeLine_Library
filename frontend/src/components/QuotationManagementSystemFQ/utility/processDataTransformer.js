...
export const transformOutsourceInjectionData = (data) => {
  ...
  const materialCostSetting = transformMaterialCostSetting(
    data.FQMaterialCostSetting
  );

  // 清理材料成本數組數據
  const materialCosts = data.FQMaterialCosts.map((item) => ({
    ...item,
  }));

  // 清理包裝成本數組數據
  const packagingCosts = data.FQPackagingCosts.map(processPackagingCost);

  // 清理委外加工費用數據
  const outPostProcessingCosts = data.FQOutPostProcessingCosts || [];

  return {
    ...baseData,
    FQMaterialCostSetting: materialCostSetting,
    FQMaterialCosts: materialCosts,
    FQPackagingCosts: packagingCosts,
    FQOutPostProcessingCosts: outPostProcessingCosts,
  };
};
...
export const transformInhousePostProcessData = (data) => {
  ...
  // 清理廠內加工費用數據
  const inPostProcessingCosts = data.FQInPostProcessingCosts || [];
  ...
  return {
    ...
    FQMaterialCosts: materialCosts,
    FQPackagingCosts: packagingCosts,
    FQInPostProcessingCosts: inPostProcessingCosts,
  };
};
...
export const transformInhouseShipmentInspectionData = (data) => {
  ...
  // 清理檢驗費用數據
  const inPostProcessingCosts = Array.isArray(data.FQInPostProcessingCosts)
    ? data.FQInPostProcessingCosts.map((item) => ({
        ...item,
      }))
    : [];
...
export const transformInhouseInjectionData = (data) => {
  ...
  // 3. 材料成本數組數據
  const materialCosts = Array.isArray(data.FQMaterialCosts)
    ? data.FQMaterialCosts.filter(Boolean).map((item) => ({
        ...item,
      }))
    : [];

  // 4. 包裝成本數組數據
  const packagingCosts = Array.isArray(data.FQPackagingCosts)
    ? data.FQPackagingCosts.filter(Boolean).map(processPackagingCost)
    : [];

  // 5. 注塑成型數據
  const injectionMoldingCost = Array.isArray(data.FQInjectionMoldingCosts)
    ? transformInjectionMoldingCosts(
        data.FQInjectionMoldingCosts.filter(Boolean)
      )
    : [];
  return {
    ...baseData,
    ...
    FQMaterialCosts: materialCosts,
    FQPackagingCosts: packagingCosts,
    FQInjectionMoldingCosts: injectionMoldingCost,
  };
};
/**
 * 根據製程類型選擇對應的轉換函數
