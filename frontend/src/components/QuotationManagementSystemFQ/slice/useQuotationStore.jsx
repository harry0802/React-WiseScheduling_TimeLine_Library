  ...
  convertProcess: (process) => {
    ...
    const baseData = {
      ...
      preInspectionLossRate:
        process.FQMaterialCostSetting?.estimatedMaterialFluctuation || 0,
      preInspectionRate:
        process.FQMaterialCostSetting?.estimatedDefectRate || 0,
      inspectionFee: process.FQMaterialCostSetting?.extractionCost || 0,
      todoItems_原物料成本:
        process.FQMaterialCosts?.map((m) => ({
          unitPrice: m.unitPrice,
          weight: m.weight,
          unit: m.unit,
        })) || [],
      todoItems_包裝材料費:
        process.FQPackagingCosts?.map((p) => ({
    ...
    switch (process.processCategory) {
      case "In-IJ(廠內成型)":
        return {
          ...
          defectRate: process.FQInjectionMoldingCosts?.[0]?.defectiveRate || 0,
          moldingCycle: process.FQInjectionMoldingCosts?.[0]?.cycleTime || 0,
          shallowPackageTime:
            process.FQInjectionMoldingCosts?.[0]?.packageTime || 0,
          holeCount: process.FQInjectionMoldingCosts?.[0]?.moldCavity || 0,
          workHourRatio:
            process.FQInjectionMoldingCosts?.[0]?.workHoursRatio || 0,
          unitPrice: process.FQInjectionMoldingCosts?.[0]?.unitPrice || 0,
        };
      ...
      case "Out-IJ(委外成型)":
        return {
          ...
          unitPrice: process.FQOutPostProcessingCosts?.[0]?.unitPrice || 0,
        };
      ...
      case "In-BE(廠內後製程)":
        return {
          ...
          workHours: process.FQInPostProcessingCosts?.[0]?.workSecond || 0,
          unitPrice: process.FQInPostProcessingCosts?.[0]?.unitPrice || 0,
        };
  ...
  devtools(
    (set, get) => ({
      ...
      calculateAll: () => {
        ...
        // 添加运输成本数据
        if (
          shippingCosts.FQFreights?.length ||
          shippingCosts.FQCustomsDuties?.length
        ) {
