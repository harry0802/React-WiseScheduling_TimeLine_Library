...
export const useScoreboardData = () => {
  ...
  const countQuery = useGetCurrentMachineStatusCountQuery();
  const proportionQuery = useGetMachineStatusProportionQuery();
  ...
  if (countQuery.data && proportionQuery.data) {
    ...
    // 合併成最終格式
    data = ["NORMAL", "TESTING", "SWITCHING", "PAUSED", "ABNORMAL"].map(
      (status) => ({
        status,
